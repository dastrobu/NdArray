...
class VectorTestsFloat: XCTestCase {
    ...
    func testSort() {
        // 0d
        do {
            let a = Vector<Float>.zeros(0)
            a.sort()
            XCTAssertEqual(a.shape, [0])
        }
        // 1d contiguous
        do {
            let a = Vector<Float>.range(to: 6)
            a.sort(order: .descending)
            XCTAssertEqual(a.dataArray, Vector<Float>.range(to: 6).dataArray.reversed())
            a.sort(order: .ascending)
            XCTAssertEqual(a.dataArray, Vector<Float>.range(to: 6).dataArray)
        }
        // 1d not aligned
        do {
            let a = Vector<Float>(Vector<Float>.range(to: 6)[..., 2])
            a.sort(order: .descending)
