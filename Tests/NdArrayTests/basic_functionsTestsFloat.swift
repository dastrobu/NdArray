...
class basic_functionsTestsFloat: XCTestCase {
    ...
    func testAbs() {
        ...
        // 1d contiguous
        do {
            let a = NdArray<Float>.range(from: -3, to: 3)
            XCTAssertEqual(abs(a).dataArray, a.dataArray.map(abs))
        }
        // 1d not aligned
        do {
            let a = NdArray<Float>.range(from: -3, to: 3)[..., 2]
            XCTAssertEqual(abs(a).dataArray, [3, 1, 1])
        }
        // 2d C contiguous
        do {
            let a = NdArray<Float>.range(from: -3, to: 3).reshaped([2, 3], order: .C)
            XCTAssertEqual(abs(a).dataArray, [3, 2, 1, 0, 1, 2])
        }
        // 2d F contiguous
        do {
            let a = NdArray<Float>.range(from: -3, to: 3).reshaped([2, 3], order: .F)
            XCTAssertEqual(abs(a).dataArray, [3, 2, 1, 0, 1, 2])
        }
        // 2d not aligned
        do {
    ...
    func testAcos() {
        ...
        // 2d effective 0d
        do {
            let a = NdArray<Float>.ones([1, 0])
            XCTAssertEqual(acos(a).shape, [1, 0])
        }
        // 1d contiguous
        do {
            let a = NdArray<Float>.range(from: 1, to: 7) / 10
            XCTAssertEqual(acos(a).dataArray, a.dataArray.map(acos))
        }
        // 1d not aligned
        do {
            let a = (NdArray<Float>.range(from: 1, to: 7) / 10)[..., 2]
            XCTAssertEqual(acos(a).dataArray, NdArray(a, order: .C).dataArray.map(acos))
        }
        // 2d C contiguous
        do {
            let a = NdArray<Float>.range(from: 0, to: 6).reshaped([2, 3], order: .C) / 10
            XCTAssertEqual(acos(a).dataArray, NdArray(a, order: .C).dataArray.map(acos))
        }
        // 2d F contiguous
        do {
            let a = NdArray<Float>.range(from: 0, to: 6).reshaped([2, 3], order: .F) / 10
    ...
    func testLog() {
        ...
        // 1d contiguous
        do {
            let a = NdArray<Float>.range(from: 1, to: 7)
        ...
        // 2d C contiguous
        do {
            let a = NdArray<Float>.range(from: 0, to: 6).reshaped([2, 3], order: .C)
            XCTAssertEqual(log(a).dataArray, NdArray(a, order: .C).dataArray.map(log))
        }
        // 2d F contiguous
        do {
            let a = NdArray<Float>.range(from: 0, to: 6).reshaped([2, 3], order: .F)
