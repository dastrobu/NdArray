...
class basic_functionsTestsFloat: XCTestCase{
    ...
    func testAbs() {
        ...
        // 1d contiguous
        do {
            let a = NdArray<Float>(rangeFrom: -3, to: 3)
            XCTAssertEqual(abs(a).dataArray, a.dataArray.map(abs))
        }
        // 1d not aligned
        do {
            let a = NdArray<Float>(rangeFrom: -3, to: 3)[..., 2]
            XCTAssertEqual(abs(a).dataArray, [3, 1, 1])
        }
        // 2d C contiguous
        do {
            let a = NdArray<Float>(rangeFrom: -3, to: 3).reshaped([2, 3], order: .C)
            XCTAssertEqual(abs(a).dataArray, [3, 2, 1, 0, 1, 2])
        }
        // 2d F contiguous
        do {
            let a = NdArray<Float>(rangeFrom: -3, to: 3).reshaped([2, 3], order: .F)
            XCTAssertEqual(abs(a).dataArray, [3, 2, 1, 0, 1, 2])
        }
        // 2d not aligned
        do {
            let a = NdArray<Float>(rangeFrom: -5, to: 4 * 3 - 5).reshaped([4, 3], order: .C)[1..., 2]
