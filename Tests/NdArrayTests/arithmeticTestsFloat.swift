...
class arithmeticTestsFloat: XCTestCase {
    ...
    func testMin() {
        ...
        // 2d not aligned
        do {
            let a = NdArray<Float>(rangeTo: 4 * 3).reshaped([4, 3], order: .C)[1..., 2]
    ...
    func testProduct() {
        ...
        // 1d contiguous
        do {
            let a = NdArray<Float>(rangeFrom: 1, to: 6)
            XCTAssertEqual(a.product(), 120)
        }
        // 1d not aligned
        do {
            let a = NdArray<Float>(rangeFrom: 1, to: 7)[..., 2]
            XCTAssertEqual(a.product(), 15)
        }
        // 2d C contiguous
        do {
            let a = NdArray<Float>(rangeFrom: 1, to: 7).reshaped([2, 3], order: .C)
            XCTAssertEqual(a.product(), 720)
        }
        // 2d F contiguous
        do {
            let a = NdArray<Float>(rangeFrom: 1, to: 7).reshaped([2, 3], order: .F)
            XCTAssertEqual(a.product(), 720)
        }
        // 2d not aligned
        do {
