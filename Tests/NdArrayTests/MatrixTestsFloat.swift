...
class MatrixTestsFloat: XCTestCase {
    ...
    func testTransposedView() {
        // 2d effective 0d
        do {
            let a = Matrix<Float>.zeros([1, 0])
            XCTAssertEqual(a.transposed().shape, [1, 0])
        }
        // 2d C contiguous
        do {
            let a = Matrix<Float>(NdArray<Float>.range(to: 6).reshaped([2, 3], order: .C))
            XCTAssertEqual(a.transposed().shape, [3, 2])
            XCTAssertEqual(a.strides, [3, 1])
            XCTAssertEqual(a.transposed().strides, [1, 3])
            XCTAssertEqual(a.transposed().dataArray, NdArray<Float>.range(to: 6).dataArray)
        }
        // 2d F contiguous
        do {
            let a = Matrix<Float>(NdArray<Float>.range(to: 6).reshaped([2, 3], order: .F))
            XCTAssertEqual(a.transposed().shape, [3, 2])
            XCTAssertEqual(a.strides, [1, 2])
            XCTAssertEqual(a.transposed().strides, [2, 1])
            XCTAssertEqual(a.transposed().dataArray, NdArray<Float>(NdArray<Float>.range(to: 6), order: .F).dataArray)
        }
        // 2d not aligned
        do {
            let a = Matrix(NdArray<Float>.range(to: 4 * 3).reshaped([4, 3], order: .C)[..., 2])
            XCTAssertEqual(a.shape, [2, 3])
            XCTAssertEqual(a.transposed().shape, [3, 2])
            XCTAssertEqual(a.strides, [6, 1])
            XCTAssertEqual(a.transposed().strides, [1, 6])
            XCTAssertEqual(a.transposed().dataArray, NdArray<Float>.range(to: 4 * 3).dataArray)
        }
    }
    ...
    func testTransposedCopy() {
        ...
        do {
            let a = Matrix<Float>.zeros([1, 0])
            let at = Matrix<Float>.empty(shape: a.shape)
            a.transposed(out: at)
        ...
        do {
            let a = Matrix<Float>(NdArray<Float>.range(to: 6).reshaped([2, 3], order: .C))
            let at = Matrix<Float>.empty(shape: a.shape.reversed())
            a.transposed(out: at)
            XCTAssertEqual(at.shape, [3, 2])
            XCTAssertEqual(a.strides, [3, 1])
            XCTAssertEqual(at.strides, [2, 1])
            XCTAssertEqual(at.dataArray, NdArray<Float>(NdArray<Float>.range(to: 6).reshaped([2, 3]), order: .F).dataArray)
        }
        // 2d C contiguous -> F contiguous
        do {
            let a = Matrix<Float>(NdArray<Float>.range(to: 6).reshaped([2, 3], order: .C))
            let at = Matrix<Float>.empty(shape: a.shape.reversed(), order: .F)
            a.transposed(out: at)
            XCTAssertEqual(at.shape, [3, 2])
            XCTAssertEqual(a.strides, [3, 1])
            XCTAssertEqual(at.strides, [1, 3])
            XCTAssertEqual(at.dataArray, NdArray<Float>(NdArray<Float>.range(to: 6).reshaped([2, 3]), order: .C).dataArray)
        }
    ...
    func testSolveAndInverted() throws {
        // 2d effective 0d
        do {
            let a = Matrix<Float>.zeros([0, 0])
            let b = Vector(NdArray<Float>.ones([0]))
        ...
        // F contiguous
        do {
            let A = Matrix(NdArray<Float>.range(from: 1, to: 5).reshaped([2, 2], order: .F))
            let b = Vector(NdArray<Float>.ones([2]))
            let x1 = try A.solve(b)
        ...
        // not aligned
        do {
            let A = Matrix(NdArray<Float>.range(to: 4 * 2).reshaped([4, 2], order: .C)[..., 2])
            let b = Vector(NdArray<Float>.ones([2]))
            let x1 = try A.solve(b)
        ...
        do {
            let A = Matrix(NdArray<Float>.range(from: 1, to: 5).reshaped([2, 2], order: .C))
            let B = Matrix(Matrix(NdArray<Float>.ones([2, 3]), order: .F)[...][..., 2])
            B[...][1].set(2.0)
            let X1 = try A.solve(B)
