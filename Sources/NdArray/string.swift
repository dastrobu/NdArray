...
fileprivate extension NdArray {
    func string(separator: String, indent: Int = 1, depth: Int = 0, formatter: (T) -> String) -> String {
        ...
        default:
            let depthSeparator: String = String(repeating: separator, count: ndim - 1)
            let a = NdArray(self)
            for i in 0..<shape[0] {
            ...
            if shape[0] > 0 {
                ...
                s.removeLast(depthSeparator.count + 1 + indent * (depth + 1))
            }
        }
        s += "]"
        return s
...
public extension String.StringInterpolation {
    ...
    mutating func appendInterpolation<T>(_ value: NdArray<T>,
        ...
        let f = formatter ?? {element in "\(element)"}
        switch style {
        case .singleLine:
            appendLiteral(value.string(separator: " ", indent: 0, formatter: f))
        case .multiLine:
            // first find the maximal field length for the items
            let n = value.reduce(1, { n, element in
                Swift.max(n, f(element).count)
            })
            let wrapperFormatter: (T) -> String = { element in
                f(element).paddingToLeft(toLength: n, withPad: " ")
            }
